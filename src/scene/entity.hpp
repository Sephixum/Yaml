...
class Entity {
  ...
  private:
    ...
    explicit Entity(std::shared_ptr<Scene> scene, entt::entity handle)
        : m_scene{scene},  //
          m_handle{handle} //
    {
        static_assert(std::is_same_v<entt::entity, decltype(handle)>,
                      "Entity handle type does not match!");
    }
  ...
  public:
    ...
    template <typename Component> [[nodiscard]] bool hasComponent() noexcept {
    ...
    auto addComponent(Args &&...args) noexcept -> decltype(auto) {
        ...
        if (hasComponent<Component>()) {
            YAML_WARN(
                std::format("Trying to add already existing component !!"));
        }

        return scene->getRegistry().emplace<Component>(
    ...
    auto getComponent() noexcept -> decltype(auto) {
        auto scene{m_scene.lock()};
        YAML_ASSERTM(scene, "Scene is null!!");
    ...
    template <typename... Components> void removeComponents() noexcept {
        auto scene{m_scene.lock()};
        YAML_ASSERTM(scene, "Scene is null!!");
        scene->getRegistry().remove<Components...>(m_handle);
    }
};
